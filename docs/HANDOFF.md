...
- 온도 지수 개편(`core/temperature.py`): 연락 텀(C, 세션 간 공백 중앙값 vs 평시)·상대 답장 길이(L) 추가, 빈도(F)는 하루치를 평시 2배로 캡, 추세(T)는 텀·길이 포함, 주간 시계열 3주 평활(0.5/0.3/0.2, 원값 `temp_raw`), 사건 임계 8°. 실데이터에서 이별 주 +20 튐이 사라지고 8/31 "연락 텀" 하락 사건이 잡힘.
