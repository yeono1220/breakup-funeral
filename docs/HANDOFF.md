...
1. ~~멀티유저 안전 아님~~ **해결(세션 격리)**: 프론트가 모든 요청에 `X-Session`(브라우저 익명 ID)을 실고, `main.py._con()`이 `DATA_DIR/sessions/<sid>.db`를 연다(업로드도 `uploads/<sid>/`). 공동묘지 폴백만 `_shared_con()`. 남은 위험: Render 무료 디스크가 휘발성이라 재시작 시 전부 사라짐(프론트는 400을 "다시 올려줘"로 안내). 원래 메모: 멀티유저 안전 아님 — 백엔드 상태(me/target/persona/메시지)가 서버 전역 SQLite `settings` 하나. 공개 URL에 동시 접속자가 2명이면 서로 덮어씀. 데모/지인 테스트는 "한 번에 한 명"이거나 세션 키(쿠키/헤더)로 격리 필요. 최우선.
2. **보안** — 세션 격리 후 `DELETE /data`는 자기 세션만. LLM 라우트(chat/summon/eulogy/curse/legends/epitaph/first_insight)는 `main.py` 미들웨어로 세션당 12회/분·120회/시, 전체 900회/시 제한(메모리, 재시작 시 리셋) → 429 + 한국어 안내. 남은 것: CORS `*`, 세션 ID(브라우저 랜덤값)를 알면 그 데이터를 읽을 수 있음 — 데모 감수, 문서화.
...
## 7-1. 2026-09-19 오후 추가분
- 온도 지수 개편(`core/temperature.py`): 평시 기준(norm)은 사용자 시작일 이후 메시지로만(`_since`, 50개 미만이면 전체). 화면의 '지금 온도'는 차트 마지막 점(평활)과 동일하게 `relationship.build`에서 덮어씀(원값 `temp_window`). 연락 텀(C, 세션 간 공백 중앙값 vs 평시)·상대 답장 길이(L) 추가, 빈도(F)는 하루치를 평시 2배로 캡, 추세(T)는 텀·길이 포함, 주간 시계열 3주 평활(0.5/0.3/0.2, 원값 `temp_raw`), 사건 임계 8°. 실데이터에서 이별 주 +20 튐이 사라지고 8/31 "연락 텀" 하락 사건이 잡힘.
- 샘플: `sample/generate.py` 대본 기반(장면 1회·시간대·톤 반응 템플릿·싸움/화해 사건). `/load_sample {me,target}`로 이름 치환.
- 소환술 첫 톡은 상대가 먼저(`summon([])` → 마지막 실제 대화 이어받기). 코치 effort medium + 툴 진행 표시 + 영수증 클릭.
- 페르소나 선택형 확장(`persona.traits`): 동물상·인상·성격/말투(최대 5)·호칭 2개·만난 경로·자주 하던 얘기·지금 연락 상태. `Funeral.persona_sheet()`가 소환술 system에 '이 사람에 대해 사용자가 알려준 것' 블록으로 넣고, 코치 system에도 JSON으로 들어감. 데모마켓 피드백("페르소나를 구체적으로") 반영.
- 모바일: 공동묘지 씬은 560px 이하에서 4:3·2줄 7기(`ROWS_M`), 탑바 한 줄 고정.
- UI: 진단서 판결 블록(묘비 제목·사인 1위·향년·이별 방식) + 부검 기록 접기, `RitualBar`(조문→입관→부적·헌화→발인) + 단계 사이 막(`.curtain`), 공동묘지 도착 시 안치 카드, 공동묘지 잔디 씬(`CemeteryScene`).
- 묘비 제목 `GET /epitaph`(어그로 한 줄, persona 컨텍스트 해시 캐시).
